...
for param in params['parameters']:
    print(param['name'])
    print(','.join(map(str, param['weights'])))
"
}

# 读取参数
mapfile -t param_data < <(read_params)
param_count=${param_data[0]}

# 解析参数数据
declare -A param_names
declare -A param_weights
for ((i=0; i<param_count; i++)); do
    param_names[$i]=${param_data[$((i*2+1))]}
    IFS=',' read -ra weights <<< "${param_data[$((i*2+2))]}"
    param_weights[$i]="${weights[@]}"
done
    ...
    while IFS=',' read -r gpu_id utilization; do
        ...
        if [[ -z "${used_gpus[$gpu_id]}" ]] && (( utilization < 10 )); then
            ...
            for ((i=0; i<param_count; i++)); do
                ...
                param_weight=${weights[$used_count]}
            ...
            eval "nohup ./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py \
                ...
                --headless \
                --num_envs 4096 \
                --video --video_length 100 --video_interval 500 \
                --device \"cuda:$gpu_id\" \

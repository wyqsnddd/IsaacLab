...
for ((i=0; i<param_count; i++)); do
    ...
    param_weight=$(yq ".parameters[$i].weights[$run_count]" "$PARAM_FILE")  # 使用对应的权重组
...
echo "执行命令: ./isaaclab.sh -p scripts/reinforcement_learning/rsl_rl/train.py --task Isaac-Velocity-Flat-D9-v0 --headless --num_envs 8192 --video --video_length 200 --video_interval 1000 --device cuda:$gpu_id"

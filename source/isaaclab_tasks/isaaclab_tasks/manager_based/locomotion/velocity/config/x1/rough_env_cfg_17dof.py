from isaaclab_assets.external_assets.assets.x1 import X1_17DOF_CFG
...
class X1Rewards17DOFCfg:
    ...
    joint_deviation_shoulder = RewTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg(
                "robot", joint_names=["Joint_Shoulder.*", "Joint_Elbow.*"]
            )
        },
    )
    ...
    leg_elbow_symmetric_reward = RewTerm(
        func = leg_arm_symmetric,
        weight= 0.2,
        params={
            "asset_cfg_leg": SceneEntityCfg(
            ...
            "asset_cfg_arm": SceneEntityCfg(
                "robot", joint_names=["Joint_Elbow_Pitch_Left", "Joint_Elbow_Pitch_Right"]
            )
        },
    )
...
@configclass
class X1RoughEnv17DofCfg(LocomotionVelocityRoughEnvCfg):
    rewards: X1Rewards17DOFCfg = X1Rewards17DOFCfg()

    def __post_init__(self):
        # post init of parent
        super().__post_init__()
        # Scene
        self.scene.robot = X1_17DOF_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
...
@configclass
class X1RoughEnv17DofCfg_PLAY(X1RoughEnv17DofCfg):
